...
def optimize_java(code):
    """
    Optimize Java code by removing dead code
    """
    try:
        optimized_code = code
        ...
        # Remove dead branches (if (false) or if (0 == 1))
        ...
        report = {
            ...
            "optimizations": [
                ...
                "Removed dead branches (if(false))",
...
def remove_unused_variables(code):
    ...
    # Pattern for Java variable declarations
    var_decl_pattern = re.compile(r'^\s*(int|char|float|double|void|long|short|boolean|String|byte)\s+\w+.*;\s*$')
...
def remove_dead_branches(code):
    """Remove dead branches like if(false)"""
    lines = code.split('\n')
    optimized_lines = []
    skip_until_brace = 0
    brace_count = 0
    ...
    while i < len(lines):
        ...
        # Check for if(false) or if(0 == 1)
        if re.match(r'^\s*if\s*\(\s*(false|FALSE|0\s*==\s*1)\s*\)', stripped):
...
def remove_unused_imports(code):
    ...
    # Keep all imports for now (conservative approach)
    # In a real implementation, you'd analyze which imports are actually used
